...
void purityEfficiencyHists(){
    ...
    // Create output file
    TString fout_name = "responseMatrixBjetMC_run.root";
...
vector<Float_t> getGenVals(Long64_t& ient, Int_t& ijet, tTree& t){
    ...
    // Have two actual Bs
    if (std::abs(t.jtNbHad[ijet]) != 2) return {0.0,0.0,0.0};
    ...
    // Reconstruct Bs using gen information
    map<int, ROOT::Math::PtEtaPhiMVector> mp;
    ReconstructBsGen(trackVectors, t, mp, ijet);
    ...
    for (auto k : finalTracks) for (auto i : k.second) for (auto j : mp) if (i==j.second) inside[k.first]++;
...
vector<Float_t> getRecoVals(Long64_t& ient, Int_t& ijet, tTree& t){
    ...
    // else if (t.jer_opt[ijet]=="up") jtpt = jtpt * t.jer_sf_up[ijet];
    // else if (t.jer_opt[ijet]=="down") jtpt = jtpt * t.jer_sf_down[ijet];

    // Reco eta and pt cuts
    if (std::abs(t.jteta[ijet]) > 2.) return {zero,zero,zero,zero};
    if ((jtpt) < 60 || (jtpt) > 150) return {zero,zero,zero,zero};

    // Skip non b-jets 
    // if (std::abs(t.jtHadFlav[ijet]) < 5) continue;
    // You can also select on the number of b hadrons 
    // if (t.jtNbHad[ijet]!=2) continue;

    // Pass tight b-tag
    if (t.discr_particleNet_BvsAll[ijet]<0.9) return {zero,zero,zero,zero};
    // Have at least two secondary vertices
    if (std::abs(t.jtNsvtx[ijet])<2) return {zero,zero,zero,zero};
    ...
    // Loop over the tracks of specific jet using trkJetId --------------------------------------------------------------
    ...
    // Group vertices if there are more than two
    ...
    if (trackVectors.size()<2) return {zero,zero,zero,zero};
    ...
    if (failed == 1) return {zero,zero,zero,zero};
    ...
    // KT CUT
    if (std::log(minPt*(ROOT::Math::VectorUtil::DeltaR(trackVectors[0], trackVectors[1])))<0) return {zero,zero,zero,zero};
    ...
    // ROOT::Math::PtEtaPhiMVector leadingJet;
    // ROOT::Math::PtEtaPhiMVector subleadingJet;
    // if (trackVectors[0].Pt()>trackVectors[1].Pt()){
    //     leadingJet=trackVectors[0];
    //     subleadingJet=trackVectors[1];
    // }
    // else {
    //     leadingJet=trackVectors[1];
    //     subleadingJet=trackVectors[0];
    // }
    // vector<ROOT::Math::PtEtaPhiMVector> leadingTracks;
    // for (auto i: finalTracks) if (abs(i.first-leadingJet.Pt())<0.01) leadingTracks=i.second;
    // auto [t_x, t_y] = computePull(leadingJet, leadingTracks);
    // double phiPull = std::atan2(t_y, t_x);
    ...
    if (counts[0]==1&&counts[1]==1){
        ...
        // double dy = subleadingJet.Rapidity() - leadingJet.Rapidity();
        // double dPhi = TVector2::Phi_mpi_pi(subleadingJet.Phi() - leadingJet.Phi());
        // double phi12  = std::atan2(dPhi, dy);
        // float theta_pull = TVector2::Phi_mpi_pi(phiPull - phi12);
        // h_phiPull->Fill(phiPull/TMath::Pi(), t.weight);
        // h_phi12->Fill(phi12/TMath::Pi(),t.weight);
        ...
        return {Zg_value,Kt_value,Rg_value,totalSVmass};
    }
    return {zero,zero,zero,zero};
}

// MAIN METHOD --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void run_MC_response()
{   
    ...
    // TFile *_file0 = TFile::Open("/data_CMS/cms/kalipoliti/herwigMC/dijet/aggrTMVA_fixedMassBug/merged_HiForestMiniAOD.root"); 
    // TString fin_name = "/data_CMS/cms/kalipoliti/herwigMC/dijet/aggrTMVA_fixedMassBug/merged_HiForestMiniAOD.root";
    TString fout_name = "histogramsSvtxGenBjetUnfoldingMC_run.root";
    tTree t(fin_name);
    ...
    TH3D *h_InvMass_vs_Zg_2b = new TH3D("h_InvMass_vs_Zg_2b", "svtx invariant mass vs zg 2b", msv_bins, msv_binsVector ,zg_bins, zg_binsVector,jtpt_bins, jtpt_binsVector);
    TH3D *h_InvMass_vs_Zg_1b = new TH3D("h_InvMass_vs_Zg_1b", "svtx invariant mass vs zg 1b", msv_bins, msv_binsVector ,zg_bins, zg_binsVector,jtpt_bins, jtpt_binsVector);
    TH3D *h_InvMass_vs_Zg_0b = new TH3D("h_InvMass_vs_Zg_0b", "svtx invariant mass vs zg 0b", msv_bins, msv_binsVector ,zg_bins, zg_binsVector,jtpt_bins, jtpt_binsVector);
    TH3D *h_InvMass_vs_Zg_Moreb = new TH3D("h_InvMass_vs_Zg_Moreb", "svtx invariant mass vs zg Moreb", msv_bins, msv_binsVector ,zg_bins, zg_binsVector,jtpt_bins, jtpt_binsVector);

    TH3D *h_InvMass_vs_Kt_2b = new TH3D("h_InvMass_vs_Kt_2b", "svtx invariant mass vs kt 2b", msv_bins, msv_binsVector ,kt_bins, kt_binsVector,jtpt_bins, jtpt_binsVector);
    TH3D *h_InvMass_vs_Kt_1b = new TH3D("h_InvMass_vs_Kt_1b", "svtx invariant mass vs kt 1b", msv_bins, msv_binsVector ,kt_bins, kt_binsVector,jtpt_bins, jtpt_binsVector);
    TH3D *h_InvMass_vs_Kt_0b = new TH3D("h_InvMass_vs_Kt_0b", "svtx invariant mass vs kt 0b", msv_bins, msv_binsVector ,kt_bins, kt_binsVector,jtpt_bins, jtpt_binsVector);
    TH3D *h_InvMass_vs_Kt_Moreb = new TH3D("h_InvMass_vs_Kt_Moreb", "svtx invariant mass vs kt Moreb", msv_bins, msv_binsVector ,kt_bins, kt_binsVector,jtpt_bins, jtpt_binsVector);

    TH3D *h_InvMass_vs_Dr_2b = new TH3D("h_InvMass_vs_Dr_2b", "svtx invariant mass vs dr 2b", msv_bins, msv_binsVector ,logrg_bins, logrg_binsVector,jtpt_bins, jtpt_binsVector);
    TH3D *h_InvMass_vs_Dr_1b = new TH3D("h_InvMass_vs_Dr_1b", "svtx invariant mass vs dr 1b", msv_bins, msv_binsVector ,logrg_bins, logrg_binsVector,jtpt_bins, jtpt_binsVector);
    TH3D *h_InvMass_vs_Dr_0b = new TH3D("h_InvMass_vs_Dr_0b", "svtx invariant mass vs dr 0b", msv_bins, msv_binsVector ,logrg_bins, logrg_binsVector,jtpt_bins, jtpt_binsVector);
    TH3D *h_InvMass_vs_Dr_Moreb = new TH3D("h_InvMass_vs_Dr_Moreb", "svtx invariant mass vs dr Moreb", msv_bins, msv_binsVector ,logrg_bins, logrg_binsVector,jtpt_bins, jtpt_binsVector);
    ...
    std::vector<TString> active_branches = {
        "weight", "evt",
        "nref", "jtpt", "jtphi", "jtm", "jteta", "lumi", "jtptCh", "discr_particleNet_BvsAll", "pthat", "ngen", // reco jet
    ...
    for (Long64_t ient = 0; ient < 1000000; ient++) {
        ...
        for (Int_t ijet = 0; ijet < t.nref; ijet++) {
            ...
            if (skipMC(jtpt, t.refpt[ijet], t.pthat)) continue;
            ...
            vector<Float_t> emptyCheck = {0.0,0.0,0.0,0.0};
            if (recoVals!=emptyCheck){
                // Fill 3D histograms used fit
                if (t.jtNbHad[ijet] == 2){
                    h_InvMass_vs_Dr_2b->Fill(recoVals[3], recoVals[2], jtpt, t.weight);
                    h_InvMass_vs_Zg_2b->Fill(recoVals[3], recoVals[0], jtpt, t.weight);
                    h_InvMass_vs_Kt_2b->Fill(recoVals[3], recoVals[1], jtpt, t.weight);
                }
                else if (t.jtNbHad[ijet] == 1){
                    h_InvMass_vs_Dr_1b->Fill(recoVals[3], recoVals[2], jtpt, t.weight);
                    h_InvMass_vs_Zg_1b->Fill(recoVals[3], recoVals[0], jtpt, t.weight);
                    h_InvMass_vs_Kt_1b->Fill(recoVals[3], recoVals[1], jtpt, t.weight);
                }
                else if (t.jtNbHad[ijet] == 0){
                    h_InvMass_vs_Dr_0b->Fill(recoVals[3], recoVals[2], jtpt,t.weight);
                    h_InvMass_vs_Zg_0b->Fill(recoVals[3], recoVals[0], jtpt, t.weight);
                    h_InvMass_vs_Kt_0b->Fill(recoVals[3], recoVals[1], jtpt, t.weight);
                }
                else if (t.jtNbHad[ijet] > 2){
                    h_InvMass_vs_Dr_Moreb->Fill(recoVals[3], recoVals[2], jtpt,t.weight);
                    h_InvMass_vs_Zg_Moreb->Fill(recoVals[3], recoVals[0], jtpt, t.weight);
                    h_InvMass_vs_Kt_Moreb->Fill(recoVals[3], recoVals[1], jtpt, t.weight);
                }
            }
    ...
    //     h->Write();
    // }
    for (auto h: {h_InvMass_vs_Kt_2b,h_InvMass_vs_Dr_2b,h_InvMass_vs_Zg_2b,h_InvMass_vs_Kt_1b,h_InvMass_vs_Dr_1b,h_InvMass_vs_Zg_1b,h_InvMass_vs_Kt_0b,h_InvMass_vs_Dr_0b,h_InvMass_vs_Zg_0b,h_InvMass_vs_Kt_Moreb,h_InvMass_vs_Dr_Moreb,h_InvMass_vs_Zg_Moreb}
    ){
        h->Write();
    }
